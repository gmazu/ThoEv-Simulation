...
class FusionBurbujas3D(ThreeDScene):
    def construct(self):
        ...
        # Subtítulo (más pequeño) + versión proporcional
        subtitulo = Text(
            "Colisión → Big Freeze",
            font="serif",
            font_size=30,
        ...
        version_sub.next_to(subtitulo, RIGHT, buff=0.15).align_to(subtitulo, DOWN)
        ...
        # 3. Subtítulo + versión subtítulo
        self.add_fixed_in_frame_mobjects(subtitulo, version_sub)
        self.play(FadeIn(subtitulo, shift=UP * 0.2), FadeIn(version_sub), run_time=1)
        ...
        num_seg_linea = 15
        alto_linea_izq = alto_barra * 0.75  # Llega hasta 75% del alto (más larga)
        ...
        alto_linea_der = alto_barra * 0.95  # Llega hasta 95% del alto (más larga)
        for j in range(num_seg_linea):
            progreso = j / num_seg_linea  # 0 arriba, 1 abajo
            opacidad_seg = 0.95 * (1 - progreso)  # Más brillo arriba
            ...
            for i in range(4):
                ...
                seg_der.move_to(pos_barra + np.array([ancho_barra/2, y_offset, 0]))
                barra_volumetrica.add(seg_der)

        # ═══════════════════════════════════════════════════════════════
        # CONTENIDO DE LA TARJETA: Iconos + Texto
        # ═══════════════════════════════════════════════════════════════

        # Icono planeta ROJO (Protocosmos Positivo)
        icono_rojo = Circle(radius=0.1, color=RED_C)
        icono_rojo.set_fill(RED_C, opacity=0.8)
        icono_rojo.set_stroke(RED_A, width=1.5, opacity=0.9)
        icono_rojo.move_to(pos_barra + np.array([-0.55, alto_barra/2 - 0.3, 0]))
        ...
        texto_rojo.next_to(icono_rojo, RIGHT, buff=0.1)

        # Icono planeta AZUL (Protocosmos Negativo)
        icono_azul = Circle(radius=0.1, color=BLUE)
        icono_azul.set_fill(BLUE, opacity=0.8)
        icono_azul.set_stroke(BLUE_A, width=1.5, opacity=0.9)
        icono_azul.move_to(pos_barra + np.array([-0.55, alto_barra/2 - 1.0, 0]))
        ...
        texto_azul.next_to(icono_azul, RIGHT, buff=0.1)
